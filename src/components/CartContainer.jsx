...
const CartContainer = ({ cartMenu, setCartMenu }) => {
    const [{  cartShow,cartItems,foodItems,total ,user}, dispatch] = useStateValue();
      ...
      <div className="w-full flex items-center justify-between p-4 cursor-pointer">
        ...
        <motion.p
          ...
          className="flex items-center gap-2 p-1 bg-gray-100 rounded-md
            ...
            text-base" onClick={cartClear}
        >
          Restaurer <RiRefreshFill />
        </motion.p>
      </div>
      {/* bottom section */}
      {cartItems && cartItems.length > 0 ? (
        ...
        justify-evenly px-8 py-2">
            <div className="w-full flex items-center justify-between">
                <p className="text-gray-400 text-lg">Somme Total</p>
                <p className="text-gray-400 text-lg">Fr {tot}</p>
            </div>
            <div className="w-full flex items-center justify-between">
                <p className="text-gray-400 text-lg">Livraison</p>
                <p className="text-gray-400 text-lg">Fr 1000</p>
            </div>
            <div className="w-full border-b border-gray-600 my-2"></div>
            <div className="w-full flex items-center justify-between">
                <p className="text-gray-200 text-xl font-semibold">Total</p>
                <p className="text-gray-200 text-xl font-semibold">Fr {tot + 1000}</p>
            </div>
           {user ?(
                <motion.button 
                whileTap={{scale:0.8}}
                ...
                text-gray-50 text-lg my-2 hover:shadow-lg">Verifier
                </motion.button>
           ):(
            <motion.button 
            whileTap={{scale:0.8}}
            type="button"
            className="w-full p-2 rounded-full bg-gradient-to-tr from-orange-400 to-orange-600 
            text-gray-50 text-lg my-2 hover:shadow-lg"> Connect pour Verifier
            </motion.button>
          ...
          <div className="w-full h-full flex flex-col items-center justify-center gap-6 ">
              ...
              <p className="text-xl text-textColor font-semibold">Ajouter au panier</p>
          </div>
      )}
